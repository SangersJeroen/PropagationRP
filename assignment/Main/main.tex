...
Because of the refraction at the interface, the two beams will get a phase difference, $\Delta \phi$. This is caused by different path length in the two media for two parallel beams. The phase change of light is given by the product of the passed time, $\Delta t$, and the angular frequency, $\omega$. Therefore, $\Delta \phi$ is given by:
...
\begin{align*}
	...
	\frac{c \: \Delta \phi}{\omega} &= \Delta x \big( n_2 \: sin(\theta_2) - n_1 \: sin(\theta_1) \big)  \\
\end{align*}

If we now let $\Delta x \rightarrow 0$, the phase difference should go to zero since the two paths will almost coincide. Therefore we get:

\begin{align*}
	\frac{c \: \Delta \phi}{\omega} = 0 &= \Delta x \big( n \: sin(\theta_2) - n \: sin(\theta_1) \big)  \\
...
We can write equtaion \ref{eq_1.6} for only the x-component:

\begin{equation}
	\frac{d}{ds} \left[ n \frac{d x}{ds} \right] = \frac{d n}{dx}
\end{equation}

If we now use that $n$ is independent of $x$, and therefore $\frac{d n}{dx} = 0$, we find:
